...
const loginSchema = {
  safeParse: (form: { email: string; password: string }) => {
    const issues = [];
    if (!form.email || !form.email.includes("@"))
      issues.push({ path: ["email"], message: "invalid_email" });
    if (!form.password || form.password.length < 6)
      issues.push({ path: ["password"], message: "password_too_short" });
...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    setErrors({ email: undefined, password: undefined, form: undefined });
    setIsLoading(true);
    ...
    try {
      ...
      if (res.ok) {
        ...
        let msg = t("invalid_email_or_password");
        try {
          const data = await res.json();
          msg = data.message || msg;
        } catch {}
        setErrors({ email: undefined, password: undefined, form: msg });
      }
    } catch (error) {
  ...
  return (
    ...
    >
      ...
      <div className="w-full lg:w-1/2 h-full flex flex-col bg-white dark:bg-black transition-colors duration-300 relative">
        {/* Top Bar with Animated Register Button */}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                ...
                <Link
                  ...
                  className="text-sm font-medium underline text-black dark:text-white hover:text-gray-600 dark:hover:text-gray-300 transition-colors duration-200"
                >
                  {t("forgot_password")}
                </Link>
              </div>
              ...
              {(errors.form || errors.email || errors.password) && (
                <div className="text-sm text-red-500 text-center">
                  {errors.form ||
                    (errors.email && <div>{t(errors.email)}</div>) ||
                    (errors.password && <div>{t(errors.password)}</div>)}
                </div>
              )}
              <div
